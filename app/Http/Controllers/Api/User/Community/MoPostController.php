...
class MoPostController extends Controller
{
    /**
     * 
     * Danh cách phòng
    ...
    public function getAll(Request $request)
    {
        ...
        $moPosts = MoPost::where('mo_posts.status', StatusMoPostDefineCode::COMPLETED)
            ...
            ->when(request('money_from') != null, function ($query) {
                $query->where([
                    ['min_money', '>=', request('money_from')],
                    ['money', '>=', request('money_from')]
                ]);
            })
    ...
    public function getOne(Request $request)
    {
        ...
        // $towerIds = MoPost::whereNotNull('tower_id')->pluck('tower_id')->toArray();
        // $splitTowerIds = array_chunk($towerIds, 50);

        // foreach ($splitTowerIds as $towerIds) {
        //     foreach ($towerIds as $towerId) {
        //         MotelUtils::handleMinMaxMoPost($towerId);
        //     }
        // }
        ...
        if ($request->user != null) {
            ...
            $moPost->previous_mo_post_id = DB::table('last_seen_mo_posts')
    ...
    public function create1(Request $request)
    {
        ...
        $moPostTitleExists = MoPost::where('title', $request->title)->where('user_id', $request->user->id)->first();
        ...
        $countPostInDay = DB::table('mo_posts')->where([
            ['user_id', $request->user->id],
        ...
        if ($request->user->has_post == false) {
            // $userExist = User::join('mo_posts', 'users.id', '=', 'mo_posts.user_id')
            //     ->where([
            //         ['users.id', $request->user->id],
            //         ['mo_posts.id', $motel_created->id]
            //     ])
            //     ->first();
            // if ($userExist != null) {
            //     $userExist->update([
        ...
        return ResponseUtils::json([
            ...
            'msg_code' => MsgCode::SUCCESS[0],
            'msg' => MsgCode::SUCCESS[1],
            'data' => MoPost::where('id', '=',   $motel_created->id)->first(),
        ]);
    }
    ...
    public function getSimilarPost(Request $request)
    {
        ...
        $moPostCompareExist = DB::table('mo_posts')->where('id', $mo_post_id)->first();
